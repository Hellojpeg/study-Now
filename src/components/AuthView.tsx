           ...
           <div className="p-12 flex flex-col justify-center overflow-y-auto">
               ...
               <div className="flex gap-4 mb-8 p-1 bg-slate-100 rounded-xl w-fit mx-auto md:mx-0">
                   <button 
                     ...
                     disabled={loading}
                     className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${mode === 'LOGIN' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'} disabled:opacity-50`}
                   >
                       Sign In
                   </button>
                   <button 
                     ...
                     disabled={loading}
                     className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${mode === 'SIGNUP' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'} disabled:opacity-50`}
                   >
                       Create Account
                   </button>
               </div>
               ...
               {error && (
                   <div className="mb-6 p-4 bg-red-50 border border-red-100 text-red-600 rounded-xl text-sm font-medium animate-pulse">
                       {error}
                   </div>
               )}
               ...
               <form onSubmit={handleSubmit} className="space-y-4">
                   ...
                   <button 
                       type="submit" 
                       disabled={loading}
                       className="w-full bg-indigo-600 text-white font-bold py-4 rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-500/30 transition-all hover:-translate-y-1 mt-6 flex items-center justify-center gap-2 disabled:bg-slate-400"
                   >
                   ...
                   <button 
                       ...
                       disabled={loading}
                       className="w-full bg-white border-2 border-slate-100 py-4 rounded-xl font-bold flex items-center justify-center gap-3 hover:border-slate-200 hover:bg-slate-50 transition-all disabled:opacity-50"
                   >
